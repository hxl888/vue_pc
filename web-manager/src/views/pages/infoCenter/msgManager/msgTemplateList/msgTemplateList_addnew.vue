<template lang="html">
    <div class="m-router-con" v-if="this.SmsTypeList.length > 0">
        <div class="addNew-box reset-dialog reset-dialog02">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" id="brandRuleForm">
                <el-form-item label="消息模板标题" prop='smsTitle'>
                  <el-input name='brandName'  :maxlength="50" v-model="ruleForm.smsTitle" class="xe-input-col3" auto-complete="off"></el-input>
                </el-form-item>
                <el-form-item label="消息模板内容" prop="smsContent" class="text_area">
                    <el-input
                    ...
                    <span class="last-num" :class="{'error': isError}">{{descTextLength}}/200</span>
                </el-form-item>
                <el-form-item label="消息变量选择" prop="freezeStatus">
                    <div>（点击后自动插入模板内容）</div>
                    <div>
                ...
                <el-form-item label="消息适用对象" prop="smsSendObj">
                ...
                <el-form-item label="消息发送类型" prop="smsSendObj">
                    <el-radio-group v-model="ruleForm.smsSendObj">
                        <el-radio v-for="(item, index) in smsSendObjlist" :label="item.value" :key='item.value'>{{item.name}}</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="是否支持跳转" prop="smsSendObj">
                    <el-radio-group v-model="ruleForm.smsSendObj">
                        <el-radio v-for="(item, index) in smsSendObjlist" :label="item.value" :key='item.value'>{{item.name}}</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item prop="smsSendObj" label="消息适用类型">
                    <el-select class="xe-input-l40" v-model="ruleForm.smsSendObj" placeholder="使用状态">
                        <el-option
        ...
        <el-dialog title="新增短信模板变量" :visible.sync="branchdilog" class="reset-dialog dialog_wid340 error-tishi">
            <el-form :model="addQuantity" :rules="rule"  label-width="70px" ref="addQuantity">
                <el-form-item prop="smsVar"label="新变量">
                    <el-input v-model="addQuantity.smsVar" :maxlength='30' type="text" class="xe-input-col2" placeholder=""></el-input>
                </el-form-item>
                <div class="el-dialog__footer">
                    <el-button type="primary" @click="addquantityName('addQuantity')">确定</el-button>
                    <el-button @click="resetQuan('addQuantity')">取 消</el-button>
                </div>
            </el-form>
        </el-dialog>
    </div>
    ...
    export default {
        data() {
            let validateBrandDesc = (rule, value, callback) => {
                if (value) {
                    if (this.descTextLength > 200) {
                        callback(new Error('输入长度超出200限制'));
                    } else {
                        callback();
                    }
                } else {
