        ...
        <div class="addNew-box reset-dialog reset-dialog02">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" id="brandRuleForm">
                <el-form-item label="短信模板标题" prop='smsTitle'>
                  <el-input size="small" name='brandName' v-model="ruleForm.smsTitle" class="xe-input-col3" auto-complete="off"></el-input>
                </el-form-item>
                ...
                <!--<el-form-item label="发送数量限制" prop="numType">-->
                    <!--<el-radio-group class="zd-radio" v-model="ruleForm.numType">-->
                        <!--<el-radio v-for="(item, index) in smsStatuslist" :label="item.value" :key='item.value'>-->
        ...
        <el-dialog title="新增短信模板变量" :visible.sync="branchdilog" class="reset-dialog dialog_wid340 error-tishi">
            <el-form :model="addQuantity" :rules="rule"  label-width="70px" ref="addQuantity">
                <el-form-item prop="smsVar"label="新变量">
                    <el-input size="small" v-model="addQuantity.smsVar" type="text" class="xe-input-col2" placeholder=""></el-input>
                </el-form-item>
...
<script>
    ...
    import {smsSendObj, smsStatus, numType} from '@/dataControl.js';
    export default {
        data() {
        ...
        methods: {
            ...
            editorList() {
                ...
                if (smsNumber) {
                    this.$http({
                        method: 'post',
                        url: '/xe-route/xe-mis/manager/smsTemplet/queryTempEdit',
                        data: {
                            smsNumber: smsNumber
                        }
                    }).then((res) => {
                        console.log('res---->', res);
                        this.ruleForm = res.data.templetEdit;
                        this.on = true;
                    }).catch((error) => {
                        console.log(error);
                    });
