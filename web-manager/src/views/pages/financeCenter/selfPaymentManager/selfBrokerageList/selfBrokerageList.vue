    ...
    <div class="m-router-con">
        <div class="u-top-search">
                <el-form :inline="true" :model="searchform" ref="searchform1" class="demo-form-inline">
                    <el-form-item prop="supName" label="">
                        <!--<el-select size="small" v-model="searchform.userCode" :filterable="true" placeholder="供应商名称">-->
                                ...
                                <!--</el-option>-->
                        <!--</el-select>-->
                         <el-input size="small" style="width: 168px"  placeholder="供应商名称" v-model="searchform.supName"> </el-input>
                    </el-form-item>
            ...
            <div class="advancedSearch" @click.stop>
                ...
                <div class="serch-content">
                    <el-form :inline="true"  label-width ='73px' :model="searchform" ref="searchform" class="demo-form-inline">
                        ...
                        <div  class="clearfix">
                             <el-form-item prop="supName" label="供应商名称">
                                <!--<el-select size="small" style="width: 168px" v-model="searchform.supName" :filterable="true" placeholder="供应商名称">-->
                                        <!--<el-option-->
                                        <!--v-for="item in venNameList"-->
                                        <!--:label="item.venName"-->
                                        <!--:key="item.userCode"-->
                                        ...
                                        <!--</el-option>-->
                                <!--</el-select>-->
                                 <el-input size="small" style="width: 168px"  placeholder="供应商名称" v-model="searchform.supName">
                                  </el-input>
                            </el-form-item>
        ...
        <div class="u-opera-btnBox">
            ...
            <span style="color: #ff4949;">&nbsp;&nbsp;&nbsp;&nbsp;*以下显示昨天自营供应商的佣金入账。分页签*金额=此商户下此sku符合时间段内产生佣金合计。不含税金额=金额/1.06。税额=金额-不含税金额</span>
        </div>
        <div>
            <el-table ref="xeFinPayRecordList" :summary-method="summaryMethod" :data="xeFinPayRecordList" show-summary border v-loading="dataListloading" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="supName" label="自营供应商名称" min-width="150"></el-table-column>
                <el-table-column prop="orgName" label="商品分类" min-width="150">
                </el-table-column>
                ...
                <el-table-column label="操作" width="95" fixed="right">
                    <template scope="scope">
                        <div class="btn-in-table">
                            <a href="javascript:;" class="s-blue" @click="viewDetials(scope.row.orgCode, scope.row.orgName, scope.row.userCode)">查看明细</a>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="cateNameTit" :visible.sync="dialogbut" class="reset-dialog">
            <div style="padding-top: 5px">
                <order-alert :orgCode="orgCode" :userCode="userCode" :dialogbut="dialogbut"></order-alert>
            </div>
        </el-dialog>
    </div>
</template>
...
//            productUrl: ''
    ...
    methods: {
        viewDetials(orgCode, orgName, userCode) { // 查看明细
            this.dialogbut = true;
            this.orgCode = orgCode;
            this.userCode = userCode;
            this.cateNameTit = `当前分类名称:${orgName}`;
        },
        summaryMethod(param) {
            ...
            for (let i = 5, len = sums.length; i < len; i++) {
                if (sums[i]) {
        ...
        listSysPayBillRecord() {
            ...
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/payBillRecord/listSysPayBillSupportRecord`,
                data: params
            }).then((res) => {
                this.dataListloading = false;
                this.pagelist.total = res.data.pageInfo.total;
                this.xeFinPayRecordList = res.data.xeFinPaySupportRecordList;
        ...
        exportDataMethod() { // 导出
            window.open(`${this.productUrl}/manager/payBillRecord/exportSysPayBillSupportRecord?${this.exportData}`);
        },
        exportPayBillDetail() { // 导出明细
            window.open(`${this.productUrl}/manager/payBillRecord/exportSysPayBillRecorDetail?${this.exportData}`);
        }
    },
    components: {
        categoryTag,
