    ...
    <div class="m-router-con">
        <div class="addNew-box basenewprop">
            <el-form :model="addformData" ref="addformData" :rules="rules" :label-width="xeLabelWidth66">
                <el-form-item label="属性类型" prop="typeName">
                  <el-input v-model="addformData.typeName" class="xe-input-col3" auto-complete="off"></el-input>
                                ...
                                <el-table-column
                                  ...
                                  width="130">
                                    <template scope="scope">
                                         <el-input v-model.trim="scope.row.propertyName"  @blur="propertyNameck(extendData1)" class="xe-input-smal90" auto-complete="off"></el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column
                                  prop="key2"
                        ...
                        <el-table
                          ...
                          border>
                              <el-table-column
                                ...
                                width="130">
                                  <template scope="scope">
                                      <el-input v-model.trim="scope.row.specName" @blur="propertyNameck(extendData2)" class="xe-input-smal90" auto-complete="off"></el-input>
                                  </template>
                              </el-table-column>
                              <el-table-column
                                prop="key2"
                ...
                <div class="dialog-footer">
                    <el-button type="primary" @click="Preservation('addformData')">保存</el-button>
        ...
        methods: {
            editEcho() { // 数据回显
                ...
                if (typeId) {
                    ...
                    this.$http({
                        method: 'post',
                        url: `${this.productUrl}/manager/type/queryTypeByTypeId`,
                        data: params
                    }).then((res) => {
                        console.log('----回显!!>', res);
                        this.editEchoData = res.data;
                        this.addformData = this.editEchoData;
//                        if (this.addformData.normalFlag) {
//                            if (this.addformData.normalFlag === 1) {
//                                this.addformData.normalFlag = true;
//                            } else if (this.addformData.normalFlag === 2) {
//                                this.addformData.normalFlag = false;
//                            }
//                        }
                        // 属性回显数据重置
                        if (this.editEchoData.typePropertyList && this.editEchoData.typePropertyList.length > 0) {
                            let extendData1 = this.editEchoData.typePropertyList;
                            for (let i in extendData1) {
                                extendData1[i].propertyValue = extendData1[i].propertyValue.split(',');
                                this.$set(extendData1[i], 'inputValue', '');
                                this.$set(extendData1[i], 'inputVisible', false);
                                if (extendData1[i].searchFlag === 1) {
                                    extendData1[i].searchFlag = true;
                                } else {
                                    extendData1[i].searchFlag = false;
                                }
                            }
                            this.extendData1 = extendData1;
                        }
                        // 规格回显数据重置
                        if (this.editEchoData.typeSpecList && this.editEchoData.typeSpecList.length > 0) {
                            let extendData2 = this.editEchoData.typeSpecList;
                            for (let i in extendData2) {
                                if (extendData2[i].specValue && extendData2[i].specValue !== '[]' && extendData2[i].specValue.length > 0) {
                                    extendData2[i].specValue = extendData2[i].specValue.split(',');
                                } else {
                                    extendData2[i].specValue = [];
                                }
                                this.$set(extendData2[i], 'inputValue', '');
                                this.$set(extendData2[i], 'inputVisible', false);
                            }
            ...
            validateNameCk(suc, err) {  // 校验属性名称是否重复
                ...
                this.$http({
                    method: 'post',
                    url: `${this.productUrl}/manager/type/changeNameCheck`,
                    data: params
                }).then((res) => {
...
//                });
            ...
            Preservation(formName) { // 提交表单
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        console.log('this.extendData1[0].propertyValue.length', this.extendData1);
                        if (this.extendData1 && this.extendData1[0].propertyValue.length > 0) {
            ...
            submitForm() {
                ...
                params.typeId = this.$route.query.typeId;
//                console.log('params.normalFlag-->', params.normalFlag);
//                if (params.normalFlag === true) {
//                    params.normalFlag = 1;
//                } else {
//                    params.normalFlag = 2;
//                }
                if (params.typePropertyList) {
                ...
                if (params.typePropertyList && params.typePropertyList.length > 0) {
                    for (let i in params.typePropertyList) {
                ...
                if (this.extendData2[0].specValue.length > 0) {
                ...
                this.$http({
                    method: 'post',
                    url: `${this.productUrl}/manager/type/updateProType`,
                    data: params
                }).then(res => {
                    console.log(res);
                    window.$wxeMessage({
