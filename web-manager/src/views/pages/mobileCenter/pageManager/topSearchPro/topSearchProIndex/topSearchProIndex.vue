            ...
            <el-table class="zq-table" ref="indexModuleList" :data="indexModuleList" border v-loading="loading">
                ...
                <el-table-column prop="modifyDate" label="操作时间" sortable></el-table-column>
                 <el-table-column prop="" label="操作" width="140" fixed="right">
                    <template scope="scope">
                        <div>
                            <el-button type="text" class="zq-restore" v-if="scope.row.menuType !== 17" @click="viewDetails(scope.row)">查看</el-button>
...
export default {
    beforeRouteEnter(to, from, next) {
        if (from.path.indexOf('topSearchPro') !== -1) {
            next(vm => {
                let topSearchProIndexSelec = window.localStorage.getItem('topSearchProIndexSelec');
                vm.getVenderModuleList(); // 表单查询
                if (topSearchProIndexSelec) {
                    vm.formsearch.cityId = parseInt(topSearchProIndexSelec);
                } else {
...
//        this.getVenderModuleList(); // 表单查询
    ...
    methods: {
        areaSelecGo() {
            this.getVenderModuleList(); // 表单查询
            window.localStorage.setItem('topSearchProIndexSelec', this.formsearch.cityId);
        },
        getAreaList() {
            let params = {};
            this.$http({
...
//                }
        ...
        getVenderModuleList(fn) {
            ...
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/cms/getTopProductModuleList`,
                data: params
            }).then((res) => {
                this.loading = false;
...
//                let topSearchProIndexSelec = window.localStorage.getItem('topSearchProIndexSelec');
//                if (topSearchProIndexSelec) {
//                    this.formsearch.cityId = parseInt(topSearchProIndexSelec);
//                } else {
//                    this.formsearch.cityId = res.data.cityId;
//                }
                fn && fn();
        ...
        viewDetails(row) {
            ...
            if (row.menuType === 14) {
                this.$router.push({name: 'topSearchProFocusMapDetails', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 15) {
                this.$router.push({name: 'topSearchProAdPositionDetails', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 16) {
                this.$router.push({name: 'topSearchProbannerDetails', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            }
        },
        editorList(row) {
            this.currentAreaList.forEach(item => {
            ...
            if (row.menuType === 14) {
                this.$router.push({name: 'topSearchProFocusMapEditor', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 15) {
                this.$router.push({name: 'topSearchProAdPositionEditor', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 16) {
                this.$router.push({name: 'topSearchProbannerEditor', params: {cityId: this.formsearch.cityId, areaName: this.areaName, menuType: row.menuType, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 17) {
                this.$router.push({name: 'topSearchProRecommend', params: {cityId: this.formsearch.cityId, areaName: this.areaName, enabledStatus: row.enabledStatus}});
            }
        },
        resetForm(formName) {
