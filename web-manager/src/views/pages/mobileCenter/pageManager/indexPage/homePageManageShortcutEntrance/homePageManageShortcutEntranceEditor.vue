    ...
    <div class="m-router-con">
        ...
        <div>
            <el-table ref="contentList" :data="contentList" border>
                <el-table-column label="图片" prop="" width="95">
                    <template scope="scope">
                        <div style="padding-top: 8px">
                            <img :src="picServer + scope.row.imgUrl" width="60" height="60">
                        </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="" label="操作" width="215" fixed="right">
                    <template scope="scope">
                        <div>
                            ...
                            <el-button type="text" v-if="contentList.length !== 1" class="zq-restore" @click="removeRow(scope.$index)">删除</el-button>
        ...
        <div class="for-itemlis" style="margin-top: 35px;padding-bottom: 0">
            <div class="item-head clearfix">
                <div class="item-tit">背景底图</div>
            </div>
        </div>
        <div class="item-uploadImg_box item-uploadImg_box_reset clearfix" v-loading='loadingUploadImg02'>
            <div class="uploadImg-btn h-uploadImgBox">
                <span>选择图片</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="upload-des" style="color: #7c8594;">四个入口时，建议上传图片尺寸为750X168。八个入口时，建议上传尺寸为750X322</i>
                <form name='form' id='formFile02'>
                    <input type="hidden" name="fileType" v-model="fileType">
                    <input type="file" name='imgFiles'  class='inputFile2' @change="inputFile2Fn('formFile02', 'secon')" accept="image/gif,image/jpeg,image/jpg,image/png">
                </form>
            </div>
            <p v-if="uploadImg_background.length" v-for='item in uploadImg_background' class="h-brand-img" style="width: 300px;margin-top: 10px">
                <img :src="picServer + item.imgName" alt="" style="width: 300px; height: 150px;">
                <span style="width: 300px;" @click.prevent='removeImg02()'>删除</span>
            </p>
        </div>
    </div>
        <!--适用区域-->
                ...
                <div class="editor_body">
                    <el-form :inline="true" :rules="rules" :model="editorForm" ref="editorForm" labelWidth="70px">
                        ...
                        <el-form-item prop="contentTitle" label="标题">
                            <el-input v-model="editorForm.contentTitle" size="small" :maxlength="8" style="width: 290px" placeholder="请输入标题"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
...
<script>
import { contentType } from '@/dataControl';
...
export default {
    ...
    data() {
        ...
        return {
            ...
            fileType: '2',
            uploadImg: [],
            uploadImg_background: [], // 背景图
            backgroundImgBut: [], // 背景图验证开关
            picServer: '',
            areaName: '',
            cityId: '',
            enabledStatus: '',
        ...
        getDetailByMenuType(cityId, menuType) {
            ...
            this.$http({
                ...
                if (res.data.backgroundImg) {
                    this.backgroundImgBut = true;
                    this.uploadImg_background = [{imgName: res.data.backgroundImg}];
                    this.editorForm.backgroundImg = res.data.backgroundImg;
                }
            }).catch((error) => {
                console.log(error);
            });
...
//                this.contentList.push({});
                ...
                window.$wxeMessage.warning('此处图片最多可添加八张，已达到数量上限，不能再进行新增');
            }
        },
        editorRow(index, row) {
...
//            let _this = this;
            if (this.editorTit === '编辑') {
                ...
                addNewList.contentTitle = this.editorForm.contentTitle;
...
//            }, 30);
        ...
        inputFile2Fn(id, secon) {
            ...
            this.$http({
                ...
                this.picServer = res.data.picServer;
                if (secon) {
                    this.uploadImg_background = imgInfoList;
                    this.loadingUploadImg02 = false;
                    this.backgroundImgBut = true;
                    this.editorForm.backgroundImg = imgInfoList[0].imgName;
                } else {
        ...
        removeImg() { // 新增修改弹框图片删除
            this.uploadImg = '';
            this.editorForm.imgUrl = '';
        },
        removeImg02() { // 背景底图图片删除
            this.backgroundImgBut = false;
            this.uploadImg_background = '';
            this.editorForm.backgroundImg = '';
        },
        goBack() {
            this.$router.go(-1);
        },
...
//            this.removeImg();
        ...
        contentListLenCk() { // 此处只支持上传四和八张图片，请修改后再次提交
            if (this.contentList.length === 4 || this.contentList.length === 8) {
                return true;
            } else {
                window.$wxeMessage.warning('此处只支持上传四和八张图片，请修改后再次提交');
                return false;
            }
        },
        saveAllForm() { // 保存
            if (this.contentListLenCk()) {
                let params = {};
                this.loading = true;
                ...
                params.backgroundImg = this.editorForm.backgroundImg;
                this.$http({
                    method: 'post',
