...
export default {
    data() {
        ...
        let checkPlateNum = (rule, value, callback) => { // 校验仓库
            var reg = /^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{5}$/;
            var res = reg.test(value);
            if (!res) {
                callback(new Error('请输入正确的车牌号'));
            } else {
                if (this.getOldcarPlateNum !== this.formData.carPlateNum) {
                    let params = {};
    ...
    methods: {
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$http({
                        method: 'post',
                        url: '/xe-route/xe-mis/manager/logisticsCar/updateLogisticsCar',
                        data: param
                    }).then((res) => {
        ...
        loadInfo() {
            this.$http({
                ...
                this.getOldcarPlateNum = res.data.responseDto.xeVenCarDTO.carPlateNum;
                this.listLineArr = this.formData.listLine;
                this.listLineArr.forEach(function(item) {
                    item.fromArea = [item.startProvinceId, item.startCityId, item.startAreaId];
                    item.endArea = [item.endProvinceId, item.endCityId, item.endAreaId];
                });
            }).catch((error) => {
                console.log(error);
            });
