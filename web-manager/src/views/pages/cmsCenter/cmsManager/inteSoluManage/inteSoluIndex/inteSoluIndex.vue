<template lang="html">
    <div class="m-router-con">
        <!--<div class="u-opera-btnBox">-->
            <!--<el-button @click="createPageBatch"><i class="iconfont icon-shouye"></i>&nbsp;生成页面</el-button>-->
            <!--<el-button @click="createPageOneKey"><i class="iconfont icon-yijianshengcheng"></i>&nbsp;一键生成首页</el-button>-->
        <!--</div>-->
        <div class="tableList">
            <el-table class="zq-table" ref="integrationData" :data="integrationData" @selection-change="handleSelectionChange" border v-loading="loading">
...
//            productUrl: ''
    ...
    created() {
        this.getIntegrationSolutionModuleList(); // 表单查询
    },
    methods: {
        enable(index, row) {
...
//                if (status === 1) {
//                    this.integrationData[this.selecRowIndex].enabledStatus = 1;
//                } else if (status === 2) {
//                    this.integrationData[this.selecRowIndex].enabledStatus = 2;
//                }
                setTimeout(() => {
                    _this.getIntegrationSolutionModuleList();
                }, 300);
            }).catch((error) => {
                console.log(error);
            });
        },
        getIntegrationSolutionModuleList() {
            let params = {};
            this.loading = true;
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/cmsPc/getIntegrationSolutionModuleList`,
                data: params
            }).then((res) => {
                this.loading = false;
                this.integrationData = res.data.moduleList;
            }).catch((error) => {
                console.log(error);
            });
        },
        viewDetails(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'inteSoluFocusMapDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 4) {
                this.$router.push({name: 'inteSoluPlatMerchantDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
        editorList(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'inteSoluFocusMapEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 4) {
                this.$router.push({name: 'inteSoluPlatMerchantEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
