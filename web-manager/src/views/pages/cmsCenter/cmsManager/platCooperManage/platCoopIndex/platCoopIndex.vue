<template lang="html">
    <div class="m-router-con">
        ...
        <!--</div>-->
        <div class="tableList">
            <el-table class="zq-table" ref="xeMallModuleData" :data="xeMallModuleData" @selection-change="handleSelectionChange" border v-loading="loading">
...
//            productUrl: ''
    ...
    created() {
        this.getPlatCooperationModuleList(); // 表单查询
    },
    methods: {
        enable(index, row) {
...
//                if (status === 1) {
//                    this.xeMallModuleData[this.selecRowIndex].enabledStatus = 1;
//                } else if (status === 2) {
//                    this.xeMallModuleData[this.selecRowIndex].enabledStatus = 2;
//                }
                setTimeout(() => {
                    _this.getPlatCooperationModuleList();
                }, 300);
            }).catch((error) => {
                console.log(error);
            });
        },
        getPlatCooperationModuleList() {
            let params = {};
            this.loading = true;
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/cmsPc/getPlatCooperationModuleList`,
                data: params
            }).then((res) => {
                this.loading = false;
                this.xeMallModuleData = res.data.moduleList;
            }).catch((error) => {
                console.log(error);
            });
        },
        viewDetails(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'platCoopFocusMapDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 3) {
                this.$router.push({name: 'platCoopMerchantCaseDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
        editorList(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'platCoopFocusMapEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            } else if (row.menuType === 3) {
                this.$router.push({name: 'platCoopMerchantCaseEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
