...
//                }
        ...
        getXeMallModuleList() {
            ...
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/cmsPc/getXeMallModuleList`,
                data: params
            }).then((res) => {
                this.loading = false;
        ...
        viewDetails(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'xyMallFocusMapDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
        editorList(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'xyMallFocusMapEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
