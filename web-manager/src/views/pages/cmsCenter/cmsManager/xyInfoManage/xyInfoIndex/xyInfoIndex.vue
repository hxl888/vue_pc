<template lang="html">
    <div class="m-router-con">
        ...
        <!--</div>-->
        <div class="tableList">
            <el-table class="zq-table" ref="xeConsultData" :data="xeConsultData" @selection-change="handleSelectionChange" border v-loading="loading">
...
//            productUrl: ''
    ...
    created() {
        this.getXeConsultModuleList(); // 表单查询
    },
    methods: {
        enable(index, row) {
...
//                if (status === 1) {
//                    this.xeConsultData[this.selecRowIndex].enabledStatus = 1;
//                } else if (status === 2) {
//                    this.xeConsultData[this.selecRowIndex].enabledStatus = 2;
//                }
                setTimeout(() => {
                    _this.getXeConsultModuleList();
                }, 300);
            }).catch((error) => {
                console.log(error);
            });
        },
        getXeConsultModuleList() {
            let params = {};
            this.loading = true;
            this.$http({
                method: 'post',
                url: `${this.productUrl}/manager/cmsPc/getXeConsultModuleList`,
                data: params
            }).then((res) => {
                this.loading = false;
                this.xeConsultData = res.data.moduleList;
            }).catch((error) => {
                console.log(error);
            });
        },
        viewDetails(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'xyInfoManDetails', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
        editorList(row) {
            if (row.menuType === 1) {
                this.$router.push({name: 'xyInfoManEditor', query: {cmsContentId: row.cmsMenuId, enabledStatus: row.enabledStatus}});
            }
        },
