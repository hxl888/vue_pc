...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        created() {
            if (this.ajaxUrl === '/xe-route/xe-mis/manager/venmanager/querySaleAreaListByUser') {
                this.loading = true;
                console.log('testthis.userCodeTmp', this.userCodeTmp);
                if (!this.userCodeTmp) {
                    return false;
                };
                this.$http({
                    ...
                    url: this.ajaxUrl,
                    data: {
                        [this.propKeys.ajaxParams]: 0,
                        userCode: this.userCodeTmp
                    }
                }).then(res => {
                    this.loading = false;
                    this.data2 = res.data.saleAreaList;
                    this.$nextTick(() => {
                        this.setDefault();
                    });
                }).catch(error => {
...
//                this.$emit('on-result-change', this.getDataResult());
            ...
            queryAreaListByParent(id, fn) {
                ...
                if (this.userCodeTmp) {
                    param.userCode = this.userCodeTmp;
                }
                this.$http({
                    method: 'post',
                    url: this.ajaxUrl,
