<template lang="html">
    <div class="m-router-con">
        <el-form :inline="true" ref="filterForm" :model="filterForm" :rules="rules" :label-width="xeLabelWidth">
            ...
            <div class="for-itemlis">
                ...
                <el-form-item label="商品名称" prop="proName">
                    <el-input class="xe-input-col3" v-model="filterForm.proName" placeholder="请输入商品名称" :maxlength="40"></el-input>
                </el-form-item>
            ...
            <div class="for-itemlis">
                ...
                <div>
                    <el-form-item label="保质期" prop="" class="clearfix">
                        <el-input style="width: 192px;" class="xe-input-col1" v-model.number="filterForm.shelfLife" @input.native="filterForm.shelfLife = parseInt($event.target.value) > 0 ? filterForm.shelfLife = parseInt($event.target.value) : ''" :maxlength='8' placeholder="请输入数字"></el-input>
                ...
                <div class="item-tabbox">
                            ...
                            <el-table-column prop="" label="规格值 (选中规格值生成商品SKU)">
                                <template scope="scope">
                                    <ul>
                                        ...
                                        >
                                            <!--{{typeSpecArr}}-->
                                            <!--{{`${scope.row.specId}-${scope.row.specName}-${item}`}}-->
                                            <!--{{scope.row}}-->
                                            <!--{{typeSpecArr.length && typeSpecArr.indexOf(`${scope.row.specId}-${scope.row.specName}-${item}`) >= 0}}-->
                                            {{item}}
                                        </li>
                                    </ul>
                                </template>
                    ...
                    <el-table class="singleTablecls" ref='singleTable' v-if='typeSpecTableData.length' :data="typeSpecTableData" border v-loading='loadingtable'>
                            ...
                            <template scope="scope">
                                <div class="skuImgUpload">
                                    ...
                                    <div class="showImg" v-if="scope.row.proMainImg">
                                        ...
                                        <img :src="imgUrlPrefix + '/product/80/80/' + scope.row.proMainImg" class="spec_pic" alt="">
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column v-if='formThead.length' v-for='(spec,sIndex) in formThead' :key='sIndex' :label="spec" :min-width="100">
                            <template scope="scope">
                        ...
                        <el-table-column
                            ...
                            <template scope="scope">
                                <div>
                                    <el-input class="xe-input-smal90" :maxlength="8" v-model="scope.row.realQuantity" @input.native="scope.row.realQuantity = parseInt($event.target.value) > 0 ? scope.row.realQuantity = parseInt($event.target.value) : ''"></el-input>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column
                ...
                <div class="item-uploadImg_box clearfix" v-loading='loadingUploadImg'>
                    <ul v-if='uploadImg2.length'>
                        <li v-for='(item, index) in uploadImg2Computed' :key='item' @mouseover.prevent="picindex = index"  @mouseout.prevent="picindex = ''">
                            <img v-if='imgUrlPrefix' :src="imgUrlPrefix + '/product/80/80/' + item.proImg" alt="">
            ...
            <div class="for-itemlis">
                ...
                <div class="form_sub_btn">
                    <el-button type="primary" @click="proReleasebtn('filterForm', 2)">下一步</el-button>
                    <el-button v-if="proDraftFlag" @click="proReleasebtn('filterForm', 1)">保存为草稿</el-button>
                </div>
            </div>
        </el-form>
        <!--商品添加弹框-->
        <div class="reset-dialog reset-dialog01">
            <el-dialog title="从商品库添加" :visible.sync="produlibadd">
                <pro-libadd @proAdd="proAddFn"></pro-libadd>
            </el-dialog>
        </div>
        <!--品牌添加弹框-->
        <div class="addNew-box reset-dialog reset-dialog02">
            <el-dialog title="新增品牌" :visible.sync="addBrand">
                <addNewBrand v-if="addBrand" class="popbox_setnewproperty"  ref="popbrand_Instance"></addNewBrand>
                <div class="dialog-footer footer_com">
                    <el-button type="primary" @click="addBrandfn">保存</el-button>
        ...
        <!--新添属性-->
        <div class="addNew-box reset-dialog reset-dialog02">
            <el-dialog title="新增属性" :visible.sync="addnewAttribute" class="noPading">
                <setnewproperties v-if="addnewAttribute" class="popbox_setnewproperty" ref="popbox_Instance"></setnewproperties>
                <div slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="addnewAttributeFn">保存</el-button>
                    <el-button @click="addnewAttribute = false">取 消</el-button>
                </div>
            </el-dialog>
        </div>
    </div>
...
<script>
    import proLibadd from './commodityLibaddAlert';
    ...
    import setnewproperties from 'views/pages/proCenter/baseSetManager/extendedAttrs/basesettingsnewproperties';
    import addNewBrand from 'views/pages/proCenter/baseSetManager/brandList/addNewBrand';
    export default {
        beforeRouteEnter(to, from, next) {
            if (to.params.proSpu) {
                window.$axios({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryVenProInfo',
                    data: {
        ...
        methods: {
            proAddFn(proSpu) {
                this.filterForm.proName = proSpu.proName; // 商品名
                let cateIds = [];
                proSpu.cateOrgCode.split('_').forEach((item) => {
                    cateIds.push(parseInt(item));
                });
                this.filterForm.cateIds = cateIds; // 商品分类
                proSpu.brandName ? this.filterForm.brandId = proSpu.brandId : this.filterForm.brandId = ''; // 商品品牌
                this.filterForm.proIntro = proSpu.proIntro; // 商品简介
                this.filterForm.frozenFlag = proSpu.frozenFlag; // 是否冻品
                this.filterForm.pricingType = proSpu.pricingType; // 计价方式
                this.pricingTypeFlag = proSpu.pricingType; // 计价方式
                this.filterForm.reserveCondition = proSpu.reserveCondition; // 贮存条件
                this.filterForm.pricingTypeInfo = proSpu.proUnit; // 计量单位
                this.pricingTypeInfoValue = proSpu.proUnit; // 计量单位
                this.filterForm.sellType = proSpu.sellType; // 售卖方式
                this.filterForm.shelfLife = proSpu.shelfLife; // 保质期
                this.filterForm.shelfLifeUnit = proSpu.shelfLifeUnit; // 保质期单位
                this.filterForm.madeinCountry = proSpu.madeinCountry;
                this.filterForm.madeinProvince = proSpu.madeinProvince;
                this.filterForm.madeinCity = proSpu.madeinCity;
                this.filterForm.madeDate = proSpu.madeDate ? new Date(proSpu.madeDate) : '';
                this.filterForm.supplierId = proSpu.supplierId;
                if (proSpu.typeId) {
                    this.filterForm.typeId = proSpu.typeId;
                    this.proKuTypeId = proSpu.typeId;
                    // 扩展属性回显
                    let proPropertyValueObj = {};
                    proSpu.propertyList.forEach(item => {
                        proPropertyValueObj['value' + item.propertyId] = item.propertyValue;
                    });
                    this.proPropertyValue = proPropertyValueObj;

                    this.querySysProSkuList(proSpu.typeId, proSpu.proSpu);
                }

                this.uploadImg2 = proSpu.imgList;
                // 定价方式
                this.priceMethodFlag = proSpu.priceMethod;

                // 配送方式
                this.deliveryTypeFlag = proSpu.deliveryType;

                // 报价过期时间
                this.failureTimeFlag = proSpu.failureTime;

//                this.prokuFlag = false;

                // 富文本信息
                this.pcDesc = proSpu.descEntity.pcDesc;
                this.appDesc = proSpu.descEntity.appDesc;
                // ===
                this.produlibadd = false;
            ...
            queryBrandList(brandId) {
                ...
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryBrandList',
                    data: {}
                }).then(res => {
            ...
            queryProTypeList() {
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryProTypeList'
                }).then(res => {
            ...
            queryProTypeInfo(id) {
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryProTypeInfo',
                    data: {
                        typeId: id
                    }
                }).then(res => {
                    ...
                    console.log('===typePropertyList===');
                    console.log(res.data.typeInfoMap.typePropertyList);
                    console.log('===typePropertyList===');
            ...
            queryVenProSkuList(id) {
                ...
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryVenProSkuList',
                    data: {
            ...
            querySysProSkuList(id, proSpu) {
                ...
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/sysprospu/querySysProSkuList',
                    data: {
                        proSpu: proSpu,
                        typeId: id
                    }
...
//                    dom.addClass('active');
            ...
            queryProBasicProperty() {
                this.$http({
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/proSpu/queryProBasicProperty'
                }).then(res => {
                    this.imgUrlPrefix = res.data.imgUrlPrefix;
            ...
            skuImgFileChange(item, event) {
                ...
                this.$http({
                    method: 'post',
                    url: '/xe-zuul/xe-route/xe-pro/product/uploadImg/proImgUpload',
                    data: formData
                }).then(res => {
                    console.log(res);
...
//                            'pcDesc': '',
                        ...
                        if (this.typeSpecTableData.length > 0) {
                            ...
                            this.typeSpecTableData.some(item => {
                                if (!item.proNum || !item.grossWeight || !item.skuPrice || !item.saleQuantity || !item.minQuantity || !item.proMainImg) {
                                    submitFlag1 = false;
                                    if (!item.proMainImg) {
                        ...
                        if (submitFlag1 && submitFlag2 && submitFlag3) {
                            ...
                            this.$http({
                                method: 'post',
                                url: '/xe-route/xe-pro/ven/proSpu/nextSavePro',
                                data: parmas
                            }).then(res => {
                                console.log(res);
                                if (submitFlag === 1) {
                                    ...
                                    this.$router.push({name: 'methodofprice', query: {'proSpu': res.data.proSpu, 'methodType': methodType, 'auditFlag': this.auditFlag, 'tempNum': this.tempNum}});
        ...
        computed: {
            ...
            typeSpecChooseResult() {
                ...
                console.log('typeSpecChooseResult');
                console.log(result);
                console.log('typeSpecChooseResult');
                let resultArrs = [];
