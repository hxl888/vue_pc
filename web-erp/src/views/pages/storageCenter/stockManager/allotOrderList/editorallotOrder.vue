        ...
        <div class="u-top-search">
            <el-form :inline="true" :model="puboderSerData.actionList" ref="puboderSerData" class="demo-form-inline">
                ...
                <el-form-item prop="sendStoreId">
                    <el-select disabled size="small" v-model="puboderSerData.actionList.sendStoreId" placeholder="调出仓库" calerable filterable class="xe-input-l40">
                        <el-option
                ...
                <el-form-item prop="receiptStoreId">
                    <el-select size="small" v-model="puboderSerData.actionList.receiptStoreId" placeholder="调入仓库" calerable filterable class="xe-input-l40">
                        <el-option
                                v-for = "item in actionList"
                                :key="item.storeId"
                                :label = "item.storeName"
                ...
                <el-form-item prop="allotRemark">
                    <el-input v-model="puboderSerData.actionList.allotRemark" size="small" placeholder="调拨备注" class="xe-input-l40"></el-input>
                </el-form-item>
            </el-form>
         </div>
         ...
         <div class="pub-dataContent">
            <div class="pub-tabDatacon">
                <el-table
                    ...
                    >
                        <template scope="props">
                            <div v-if="props.row.proSpecValue">
                                {{props.row.proSpecValue}}
                            </div>
                            <div v-else>无</div>
                        </template>
                    </el-table-column>
                    ...
                    >
                        <template scope="props">
                            <div v-if="props.row.proPrice">
                                {{parseFloat(props.row.proPrice).toFixed(2)}}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
                    ...
                    >
                        <template scope="scope">
                            <div v-if="scope.row.proPrice && scope.row.allotNums">
                                {{parseFloat((scope.row.proPrice ? scope.row.allotNums : 0) * (scope.row.allotNums ? scope.row.allotNums : 0)).toFixed(2)}}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
                    ...
                    <div class="tab-editrequise">
                        <el-form :inline="true" :model="formInline" ref="formInline">
                            ...
                            <el-form-item prop="proName">
                                <el-input v-model="formInline.proName" size="small" placeholder="盘点单号" class="xe-input-l40"></el-input>
                            </el-form-item>
                            ...
                            <div class="table-editrequise">
                                <el-table
                                    ...
                                    <el-table-column type="selection" min-width="34" class-name="selectionTd">
                                        ...
                                        <el-table-column label="规格" prop="proSpecValue" min-width="85">
                                            <template scope="props">
                                                <div v-if="props.row.proSpecValue">
                                                    {{props.row.proSpecValue}}
                                                </div>
                                        ...
                                        <el-table-column label="可用库存" prop="" min-width="85">
                                            <template scope="scope">
                                                <div v-if="scope.row.actualInv">
                                                    {{(scope.row.actualInv ? scope.row.actualInv : 0) - (scope.row.lockedInv ? scope.row.lockedInv : 0)}}
                                                </div>
                                            </template>
                                        </el-table-column>
                                </el-table>
    ...
    export default {
        data() {
            return {
                ...
                rule: {
                    allotNums: [
                        { required: true, message: '请填写调拨数量', trigger: 'blur' }
                    ]
                },
                modifyallot: {
                    allotNums: ''
        ...
        methods: {
            ...
            resetForm() {},
            onSubmit() {
                if (this.puboderSerData.actionList.receiptStoreId === '') {
                    this.$alert('请先选择调入仓库', {
                        type: 'info'
                    });
                } else {
                    if (this.puboderSerData.actionList.receiptStoreId === this.puboderSerData.actionList.sendStoreId) {
                        ...
                        if (this.seleCollection.length === 0) {
                            ...
                            this.$confirm('您确认保存当前调拨单数据吗？', '提示', {
                                ...
                                this.puboderSerData.actionList.allotDetailList = this.seleCollection;
                                console.log(this.puboderSerData.actionList);
                                let data = {};
                                data = this.puboderSerData.actionList;
                                this.$http({
                                    method: 'post',
                                    url: '/xe-route/xe-erp/allotManager/updateStoreAllot',
                                    data: data
                                }).then((res) => {
                                    console.log('添加成功');
                                    window.$wxeMessage({
            ...
            endDateStrFn() {},
            addAllocation() {
                if (this.puboderSerData.actionList.sendStoreId) {
                    let data = {};
                    data = this.formInline;
                    data.storeId = this.puboderSerData.actionList.sendStoreId;
            ...
            selectionShop(n) {
                ...
                if (this.selectedCollection.length === 0) {
                    ...
                    if (n) {
                        this.puboderSerData.actionList.allotDetailList = this.selectedCollection;
                        this.dialogVisible = false;
                    } else {
                        this.puboderSerData.actionList.allotDetailList = this.selectedCollection;
                    }
                }
                this.puboderSerData.actionList.allotDetailList.forEach((item) => {
            ...
            deletAllot(index) {
                this.$confirm('您确认删除当前调拨商品吗？', '提示', {
                    ...
                    this.puboderSerData.actionList.allotDetailList.splice(index, 1);
                    window.$wxeMessage({
                        type: 'success',
                        message: '完成删除!'
                    });
