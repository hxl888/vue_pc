...
export default {
    ...
    methods: {
        ...
        AuditedMethod(checkStatus, call) { // 审核通过
            ...
            let ajaxUrl = '';
            if (checkStatus === 5) {
                params.financeRejectReason = this.rejectext;
                ajaxUrl = '/xe-route/xe-erp/supplier/financeReject';
            } else {
                ajaxUrl = '/xe-route/xe-erp/supplier/financeCheck';
            }
            this.$http({
                method: 'post',
                url: ajaxUrl,
                ...
                setTimeout(function () {
                    _this.$router.push({name: 'supplierCheckListIndex'});
                }, 300);
            }).catch((error) => {
                console.log(error);
            });
