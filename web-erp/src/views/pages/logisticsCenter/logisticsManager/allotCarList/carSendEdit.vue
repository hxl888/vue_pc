            ...
            <div class="for-itemlis">
                ...
                <div>
                    <el-form-item label="店铺名称:">
                        {{formData.caStoreName}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="配送方式:">
                        {{formData.shipType | shipTypeToText}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="客户指定车辆:">
                        {{formData.shipCarDes | noneToText}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="买家备注:">
                        {{formData.orderMark | noneToText}}
                    </el-form-item>
                </div>
...
export default {
    ...
    methods: {
        ...
        saveNewCar() {
            this.$refs.formAddCar.validate((valid) => {
                if (valid) {
                    ...
                    param.caAreaId = this.formData.caAreaId;
                    param.caCityId = this.formData.caCityId;
                    param.caProvinceId = this.formData.caProvinceId;
                    param.cityId = this.formData.cityId;
                    param.provinceId = this.formData.provinceId;
                    param.areaId = this.formData.areaId;
                    this.$http({
                        method: 'post',
        ...
        loadLineInfo(pageNum) {
            ...
            param.provinceId = this.formData.provinceId;
            param.areaId = this.formData.areaId;
            param.logtsCode = this.formData.logtsCode;
            this.$http({
                method: 'post',
        ...
        saveCarSend() {
            let param = {
                ...
                logtCode: this.formData.logtsCode,
                caAreaId: this.formData.caAreaId,
                caCityId: this.formData.caCityId,
                caProvinceId: this.formData.caProvinceId,
        ...
        loadInfo() {
            ...
            this.$http({
                method: 'post',
                url: '/xe-route/xe-erp/logistics/toUpdateSendCarPage',
                data: {
                    logtsCode: this.$route.query.logtsCode
                }
            }).then((res) => {
                this.formData = res.data.sendCarLogistics;
                this.shopInfoTmp = {
                    carPlateNum: this.formData.carPlateNum,
                    mobilePhone: this.formData.mobilePhone,
                    contactPerson: this.formData.contactPerson,
                    carId: this.formData.carId
                };
                this.loadAddr = this.formData.loadAddr;
                this.value1 = this.formData.loadTime;
                this.unloadAddr = this.formData.unloadAddr;
                this.value2 = this.formData.unloadTime;
