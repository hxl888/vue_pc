<template lang="html">
    <div class="m-router-con">
    	<el-form class="detailsCar"  ref = "formData" :model = "formData" label-width = "95px">
            <!-- {{shopInfoTmp}} -->
            <div class="for-itemlis" v-if="formData.sendCarLogistics">
                <h4 class="tit-salesApply">基本信息</h4>
                <div>
                    <el-form-item label="物流单号:">
                        {{formData.sendCarLogistics.logtsCode}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="客户经理:">
                        {{formData.sendCarLogistics.salesmanName}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="客户经理手机:">
                        {{formData.sendCarLogistics.salesmanPhone}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="收货人:">
                        {{formData.sendCarLogistics.caConsignee}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="收货人电话:">
                        {{formData.sendCarLogistics.caPhone}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="收货地址：">
                        {{formData.sendCarLogistics.receiptAddress}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="店铺名称:">
                        {{formData.sendCarLogistics.caStoreName}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="配送方式:">
                        {{formData.sendCarLogistics.shipType | shipTypeToText}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="客户指定车辆:">
                        {{formData.sendCarLogistics.shipCarDes | noneToText}}
                    </el-form-item>
                </div>
                <div>
                    <el-form-item label="买家备注:">
                        {{formData.sendCarLogistics.orderMark | noneToText}}
                    </el-form-item>
                </div>
    ...
    methods: {
        ...
        saveNewCar() {
            this.$refs.formAddCar.validate((valid) => {
                console.log('testvalid', valid);
                if (valid) {
                    let param = this.formAddCar;
                    param.caAreaId = this.formData.sendCarLogistics.caAreaId;
                    param.caCityId = this.formData.sendCarLogistics.caCityId;
                    param.caProvinceId = this.formData.sendCarLogistics.caProvinceId;
                    param.cityId = this.formData.sendCarLogistics.cityId;
                    param.provinceId = this.formData.sendCarLogistics.provinceId;
                    param.areaId = this.formData.sendCarLogistics.areaId;
                    this.$http({
                        method: 'post',
        ...
        chooseCar() {
            this.loadLineInfo(this.xePageInfo.pageNum);
        ...
        loadLineInfo(pageNum) {
            ...
            param.provinceId = this.formData.sendCarLogistics.provinceId;
            param.areaId = this.formData.sendCarLogistics.areaId;
            param.logtsCode = this.formData.sendCarLogistics.logtsCode;
            this.$http({
                method: 'post',
                url: '/xe-route/xe-erp/logistics/queryCarList',
                data: param
            }).then((res) => {
        ...
        saveCarSend() {
            let param = {
                ...
                logtCode: this.formData.sendCarLogistics.logtsCode,
                caAreaId: this.formData.sendCarLogistics.caAreaId,
                caCityId: this.formData.sendCarLogistics.caCityId,
                caProvinceId: this.formData.sendCarLogistics.caProvinceId,
                cityId: this.formData.sendCarLogistics.cityId,
                provinceId: this.formData.sendCarLogistics.provinceId,
                areaId: this.formData.sendCarLogistics.areaId,
            ...
            this.$http({
                method: 'post',
                url: '/xe-route/xe-erp/logistics/saveSendCarInfo',
                data: param
            }).then((res) => {
                let _this = this;
        ...
        loadInfo() {
            console.log('test', this.$route.query.logtsCode);
            this.$http({
                method: 'post',
                url: '/xe-route/xe-erp/logistics/toSendCarPage',
                data: {
                    logtsCode: this.$route.query.logtsCode
                }
            }).then((res) => {
