    ...
    <div class="m-router-con">
        ...
        <el-form :inline="true"
                ...
                <div>
                 ...
                 <div>
                    <el-form-item label="计价方式" prop="">
                        <el-radio-group v-model.number="insertFareForm.pricingType">
            ...
            <div v-if="insertFareForm.fareMailFlag === 2">
                ...
                <div class="newTem-flor02" style="padding-bottom: 0;">
                    <!-- <div class="editor-fist clearfix">
                        <span>默认始发地运费&nbsp;&nbsp;</span>
                        <el-input  class = "freight-txt xe-input-smal90_hei30" v-model.number="insertFareForm.firstPro" placeholder=""></el-input>
                        <span>&nbsp;&nbsp;{{unit}}内，&nbsp;&nbsp;</span>
                        <el-input  class = "freight-txt xe-input-smal90_hei30 f-fixedOne" v-model.number="insertFareForm.firstPrice" placeholder=""></el-input>
                        <span>元，每增加&nbsp;&nbsp;</span>
                        <el-input  class = "freight-txt xe-input-smal90_hei30" v-model.number="insertFareForm.incrPro" placeholder=""></el-input>
                        <span>&nbsp;&nbsp;{{unit}}，增加运费</span>
                        <el-input  class = "freight-txt xe-input-smal90_hei30 f-fixedOne" v-model.number="insertFareForm.incrPrice" placeholder=""></el-input>
                        <span>&nbsp;&nbsp;元</span>
                    </div> -->
                    <!--表格区域-->
                    <div class="editor-content clearfix">
                        <div class="addres-nav fl">
                            <ul>
                                <li v-for="(item, adrIndex) in insertFareForm.fromCityList"  class="adres-lis adres-lisArea" :key="adrIndex">
                                    ...
                                    <p :title="item.fromProvinceName+'-'+item.fromCityName">{{item.fromProvinceName}}-{{item.fromCityName}}</p>
                        ...
                        <div class="editor-box fl" v-for="(item, adrIndex) in insertFareForm.fromCityList" :key="adrIndex">
                            ...
                            <div class="editor-oper-content">
                                ...
                                <div class="add-editorData" @click="addRules(item.fareDetailList)"><span class="bord-radio"><i class="iconfont icon-zengjia"></i></span>&nbsp;添加运费规则</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="u-next f-next">
                <el-button type="primary" :loading='nextDisabled' @click="saveForm('insertFareForm')">保存</el-button>
                <el-button @click="backPage">返回</el-button>
            </div>
        </el-form>
        <!--弹框-->
        <div class="com-address">
            <el-dialog title="添加地址" class="reset-dialog" :visible.sync="subArea">
                <el-form :inline="true"
                    class="addressForm"
                    ref = "areaSelect"
                    :model = "areaSelect"
    ...
    export default {
        ...
        methods: {
                                    ...
                                    if (!(r.test(fareFrom[i].firstPro) && r.test(fareFrom[i].incrPro) && rp.test(fareFrom[i].firstPrice) && rp.test(fareFrom[i].incrPrice))) {
                                        ...
                                        if (!subFromList.length) { // 没有运费rules 进入下一条 最后一个则提交
                                            ...
                                            for (var j = 0; j < subFromList.length; j++) {
                                                if (!(r.test(subFromList[j].firstPro) && r.test(subFromList[j].incrPro) && rp.test(subFromList[j].firstPrice) && rp.test(subFromList[j].incrPrice) && subFromList[j].useArea.length > 0)) {
                                                    this.$xeMessage.warning('认真填写运费规则！');
                                                    // return false;
                                                    break;
                                                } else {
                                                    console.log('subFromList.length', subFromList.length);
            ...
            commitForm() {
                ...
                this.$http({ // 添加运费模板
                    method: 'post',
                    url: '/xe-route/xe-pro/ven/fare/insertFareTemplate',
                    data: data
                }).then((res) => {
                    if (res.status === '0000') {
...
<style lang="scss">
    // .editor-box .xe-input-smal90_hei30{
    //     width: 90px;
    // }
