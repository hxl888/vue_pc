    ...
    <div class="u-top-search">
     ...
     <div class="goodsTable">
          <el-table highlight-current-row style="width: 100%" border :data="operatorLogList">
              ...
              <el-table-column label="操作时间" min-width="140" prop="operatorTime">
...
export default {
    ...
    created() {
        console.log('test');
        let logType = {
            callCenterlogList: 'erpAfterSale', // 售后日志列表
            storageCenterLogList: 'erpStoreCenter', // 仓储中心
            stockLogList: 'erpStoreCenter', // 仓储中心2
            orderCenterLogList: 'erpOrder', // 订单日志列表
            supplierLogList: 'erpSupplier', // 供货商
            procurementCenterLogList: 'erpPurch' // 采购中心
        };
    ...
    methods: {
        ...
        enquiryForm(pageNum, searchform) {
            ...
            this.$http({ // 查询历史记录
                method: 'post',
                url: '/xe-route/xe-erp/order/queryOrderInfoLog',
                data: data
            }).then((res) => {
                this.operatorLogList = res.data.logisticsList;
