<template lang='html'>
    <div class='m-submenu' v-if='subMenuData.length'>
        <div class="m-submenu-con">
            <el-menu class="el-menu-vertical-demo" :default-active='activeId' :router="true" :default-openeds="openedsArr">
                <template v-for='(item, pindex) in subMenuData'>
                    <template v-if='item.subMenu.length'>
                        <el-submenu v-for='(subItem, cindex) in item.subMenu' :index="item.id + ''">
                            <template slot="title">{{item.menuName}}</template>
                            <el-menu-item :route='{path: subItem.url}' :index="subItem.id + ''">{{subItem.menuName}}</el-menu-item>
                        </el-submenu>
                    </template>
